document.getElementById('increase-font-size').addEventListener('click', function() {
    ...
    if (currentClass === '') {
      document.body.className = 'font-size-1';
    } else if (currentClass === 'font-size-1') {
      document.body.className = 'font-size-2';
    } else {
      // Reset if needed
...
! function (t, e, i, n) {
    ...
    $(this).on("mousewheel DOMMouseScroll", function (e) {
        ...
        var t = $(".work").find(".is-active"),
            n = $(".work").children().index(t),
            s = $(".work").children().length - 1,
            r = s;
        e(s), i(n, r, s)
    });
...
let tamanhoFonteAtual = 1; // Representa um tamanho de fonte inicial de 'normal'

aumentarBtn.addEventListener("click", () => {
  tamanhoFonteAtual += 0.1;
  corpo.style.fontSize = `${tamanhoFonteAtual}em`;
});
